...
class Factory
{
     ...
     * @param Dompdf $dompdf The dependency instance
     *
    ...
    public static function decorate_frame(Frame $frame, Dompdf $dompdf, ?Frame $root = null): ?AbstractFrameDecorator
    {
        ...
        if ($node->nodeName === "img") {
            $style->set_prop("display", "-dependency-image");
            $decorator = "Image";
            $reflower = "Image";
        }
        ...
        if ($display === "list-item") {
            ...
            if ($parent_node && $parent_node instanceof \DOMElement) {
                if (!$parent_node->hasAttribute("dependency-children-count")) {
                    $xpath = new DOMXPath($xml);
                    $count = $xpath->query("li", $parent_node)->length;
                    $parent_node->setAttribute("dependency-children-count", $count);
                }

                if (is_numeric($node->getAttribute("value"))) {
                    $index = intval($node->getAttribute("value"));
                } else {
                    if (!$parent_node->hasAttribute("dependency-counter")) {
                        $index = ($parent_node->hasAttribute("start") ? $parent_node->getAttribute("start") : 1);
                    } else {
                        $index = (int)$parent_node->getAttribute("dependency-counter") + 1;
                    }
                }

                $parent_node->setAttribute("dependency-counter", $index);
                $bullet_node->setAttribute("dependency-counter", $index);
            }

            $new_style = $dompdf->getCss()->create_style();
            $new_style->set_prop("display", "-dependency-list-bullet");
